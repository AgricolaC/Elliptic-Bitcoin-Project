...
### Temporal Conditioning is One-Step-Ahead (P0a)
When classifying snapshot τ, the conditioning hidden state `h_τ` is built **only from steps strictly before τ** (`_onestep_blocks` + `_temporal_state` in `evaluation/temporal_validation.py`: the state classifying τ runs the embedder over `train_block + [τ-1]`, excluding τ). Calibration is symmetric — the state classifying τ-1 excludes τ-1 (runs over `train_block`). This removes the self-conditioning confound (review #2): the model never sees the snapshot it is scoring, even though the embedder is label-free. An earlier "filtering" variant *included* τ's own embedding; it was reverted because, for the central thesis test ("is temporal memory toxic under the τ=43 shift?"), letting the state peek at τ confounds memory effects with same-step information.
