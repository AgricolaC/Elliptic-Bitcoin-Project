...
def build_section_7() -> list:
    ...
    cells.append(_code(
        ...
        "# Draw Low-Confidence bands from union across all λ variants\n"
        "lc_taus = set()\n"
        "lc_taus.update(base[base['Low_Confidence']]['Tau'].tolist())\n"
        "for lam_val2 in [0.05, 0.25, 0.5]:\n"
        "    sub2 = df_ts[\n"
        "        select(df_ts, family='SGC', K=2, Dir=True, Topo='early', lam=lam_val2)\n"
        "        & (df_ts['_variation'] == 'Base')\n"
        "    ].drop_duplicates('Tau')\n"
        "    if not sub2.empty:\n"
        "        lc_taus.update(sub2[sub2['Low_Confidence']]['Tau'].tolist())\n"
        "for tau in sorted(lc_taus):\n"
        "    ax.axvspan(tau - 0.45, tau + 0.45, alpha=0.12, color='grey')\n\n"
    ...
    cells.append(_code(
        "# df_ts is loaded in the preceding Plot A cell — run that cell first\n"
